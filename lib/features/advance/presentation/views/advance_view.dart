...
import 'package:pakkahishab/features/advance/presentation/viewmodels/advance_viewmodel.dart';
import 'package:pakkahishab/features/advance/presentation/widgets/advance_appbar_back_with_search.dart';
...
class AdvanceView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AdvanceAppbarBackWithSearch(title: "Advances"),
      body: SafeArea(
        child: Consumer(
          builder: (context, ref, child) {
            return RefreshIndicator(
              ...
              onRefresh: () {
                return ref
                    .read(advanceViewModelProvider.notifier)
                    .refreshSales();
              },
              child: Consumer(
                builder: (outerContext, ref, child) {
                  final incomeState = ref.watch(advanceViewModelProvider);

                  if (incomeState.loading) {
                    return Center(child: loader);
                  }
                  if (incomeState.advanceList.isEmpty) {
                    return Center(child: Text("No Advances"));
                  }
                  return Column(
                    children: [
                      Container(
                              ...
                              Expanded(
                                child: Column(
                                  children: [
                                    ...
                                    Text(
                                      incomeState.advanceList.isEmpty
                                          ? "0"
                                          : ref
                                                .watch(advanceViewModelProvider)
                                                .advanceList
                                                .length
                                                .toString(),
                                      style: AppTextStyle.labelLarge,
                      ...
                      Expanded(
                        child: ListView.builder(
                          itemCount: incomeState.advanceList.length,
                          itemBuilder: (context, index) {
                            final item = incomeState.advanceList[index];
...
class _SalesPaginationState extends ConsumerState<SalesPagination> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final currentPage = ref.read(advanceViewModelProvider).currentPage;
      if (currentPage > 1) {
  ...
  @override
  Widget build(BuildContext context) {
    final purchaseState = ref.watch(advanceViewModelProvider);
    final notifier = ref.read(advanceViewModelProvider.notifier);
