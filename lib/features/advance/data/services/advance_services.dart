...
final advanceServiceProvider = Provider<AdvanceServices>(
  (ref) => AdvanceServices(),
);

class AdvanceServices {
  Future<Map<String, dynamic>> getAdvanceItem({
    ...
    final String url = "${Urls.baseUrl}Get_Advance_Amount/";
    try {
  ...
  Future<Map<String, dynamic>> getAdvanceHead({
    ...
    final url =
        "${Urls.baseUrl}Get_Advance_Head/?mobile=$phone&password=$pin&school_code=$code";
    Dio dio = Dio();
    try {
      final response = await dio.get(url);
