...
import 'package:pakkahishab/features/bank/presentation/viewmodels/bank_viewmodel.dart';
import 'package:pakkahishab/features/bank/presentation/widgets/bank_appbar_back_with_search.dart';
...
class BankView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: BankAppbarBackWithSearch(title: "Bank"),
      body: SafeArea(
        child: Consumer(
          builder: (context, ref, child) {
            return RefreshIndicator(
              ...
              onRefresh: () {
                return ref.read(bankViewModelProvider.notifier).refreshSales();
              },
              child: Consumer(
                builder: (outerContext, ref, child) {
                  final incomeState = ref.watch(bankViewModelProvider);

                  if (incomeState.loading) {
                    return Center(child: loader);
                  }
                  if (incomeState.bankAmountList.isEmpty) {
                    ...
                    children: [
                              ...
                              Expanded(
                                child: Column(
                                  children: [
                                    ...
                                    Text(
                                      incomeState.bankAmountList.isEmpty
                                          ? "0"
                                          : ref
                                                .watch(bankViewModelProvider)
                                                .bankAmountList
                                                .length
                                                .toString(),
                                      style: AppTextStyle.labelLarge,
                      ...
                      Expanded(
                        child: ListView.builder(
                          itemCount: incomeState.bankAmountList.length,
                          itemBuilder: (context, index) {
                            final item = incomeState.bankAmountList[index];
                            final parsedDate = DateTime.parse(item.vDate);
...
class _SalesPaginationState extends ConsumerState<SalesPagination> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final currentPage = ref.read(bankViewModelProvider).currentPage;
      if (currentPage > 1) {
  ...
  @override
  Widget build(BuildContext context) {
    final purchaseState = ref.watch(bankViewModelProvider);
    final notifier = ref.read(bankViewModelProvider.notifier);
