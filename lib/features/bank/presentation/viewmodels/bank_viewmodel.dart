...
final bankViewModelProvider =
    NotifierProvider.autoDispose<BankNotifier, BankState>(
      () => BankNotifier(),
    );
...
final class BankState {
  ...
  final String pin;
  final String offset;
  final List<BankAmountItem> bankAmountList;
  ...
  BankState copyWith({
    ...
    String? pin,
    String? offset,
    List<BankAmountItem>? bankAmountList,
    String? productId,
    ...
    return BankState(
      ...
      bankAmountList: bankAmountList ?? this.bankAmountList,
...
class BankNotifier extends Notifier<BankState> {
  late final BankRepository _repo;

  @override
  BankState build() {
    _repo = ref.read(bankRepositoryProvider);
  ...
  Future<void> fetchBankAmountItem({
    ...
    try {
      ...
      final newItems = items
          .map<BankAmountItem>((e) => BankAmountItem.fromJson(e))
          .toList();

      state = state.copyWith(
        bankAmountList: newItems,
        offset: newOffset.toString(),
        currentPage: newPage,
        hasMore: hasMore,
      );
