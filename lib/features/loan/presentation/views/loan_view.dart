...
import 'package:pakkahishab/features/loan/presentation/viewmodels/loan_viewmodel.dart';
import 'package:pakkahishab/features/loan/presentation/widgets/loan_appbar_back_with_search.dart';
...
class LoanView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: LoanAppbarBackWithSearch(title: "Loans"),
      body: SafeArea(
        child: Consumer(
          builder: (context, ref, child) {
            return RefreshIndicator(
              ...
              onRefresh: () {
                return ref.read(loanViewModelProvider.notifier).refreshSales();
              },
              child: Consumer(
                builder: (outerContext, ref, child) {
                  final incomeState = ref.watch(loanViewModelProvider);

                  if (incomeState.loading) {
                    return Center(child: loader);
                  }
                  if (incomeState.loanList.isEmpty) {
                    ...
                    children: [
                              ...
                              Expanded(
                                child: Column(
                                  children: [
                                    ...
                                    Text(
                                      incomeState.loanList.isEmpty
                                          ? "0"
                                          : ref
                                                .watch(loanViewModelProvider)
                                                .loanList
                                                .length
                                                .toString(),
                                      style: AppTextStyle.labelLarge,
                      ...
                      Expanded(
                        child: ListView.builder(
                          itemCount: incomeState.loanList.length,
                          itemBuilder: (context, index) {
                            final item = incomeState.loanList[index];
...
class _SalesPaginationState extends ConsumerState<SalesPagination> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final currentPage = ref.read(loanViewModelProvider).currentPage;
      if (currentPage > 1) {
  ...
  @override
  Widget build(BuildContext context) {
    final purchaseState = ref.watch(loanViewModelProvider);
    final notifier = ref.read(loanViewModelProvider.notifier);
