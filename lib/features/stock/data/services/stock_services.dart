...
final stockServiceProvider = Provider<StockServices>(
  (ref) => StockServices(),
);
...
class StockServices {
  Future<Map<String, dynamic>> getStocks({
    ...
    final String url = "${Urls.baseUrl}Get_Stock/";
    // https://erp.bdtender.tech:8443/ords/dev/PakkahisabApp/Get_Stock/?school_code=3&mobile=01602441508&password=nadim&product_id=31&offset=0&limit=10
    try {
  ...
  Future<Map<String, dynamic>> getStockProductName({
    ...
    final url = "${Urls.baseUrl}Get_Product_Name/?mobile=$phone&password=$pin&school_code=$code";
    Dio dio = Dio();
    try {
      final response = await dio.get(url);
      print(url);
