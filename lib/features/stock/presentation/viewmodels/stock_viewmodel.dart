...
import 'package:pakkahishab/features/stock/data/models/stock_model.dart';
import 'package:pakkahishab/features/stock/data/models/stock_product_model.dart';
import 'package:pakkahishab/features/stock/data/repositories/stock_repository.dart';

final stockViewModelProvider =
    NotifierProvider.autoDispose<StockNotifier, StockState>(
      () => StockNotifier(),
    );
...
class StockNotifier extends Notifier<StockState> {
  late final StockRepository _repo;

  @override
  StockState build() {
    _repo = ref.read(stockRepositoryProvider);
  ...
  Future<void> fetchStock({
    ...
    try {
      ...
      final newItems = items
          .map<StockItem>((e) => StockItem.fromJson(e))
          .toList();
