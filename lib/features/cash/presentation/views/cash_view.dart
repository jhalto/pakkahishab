...
import 'package:pakkahishab/features/cash/presentation/viewmodels/cash_viewmodel.dart';
import 'package:pakkahishab/features/cash/presentation/widgets/cash_appbar_back_with_search.dart';
...
class CashView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: CashAppbarBackWithSearch(title: "Cash"),
      body: SafeArea(
        child: Consumer(
          builder: (context, ref, child) {
            return RefreshIndicator(
              ...
              onRefresh: () {
                return ref.read(cashViewModelProvider.notifier).refreshSales();
              },
              child: Consumer(
                builder: (outerContext, ref, child) {
                  final incomeState = ref.watch(cashViewModelProvider);

                  if (incomeState.loading) {
                    return Center(child: loader);
                  }
                  if (incomeState.cashList.isEmpty) {
                    ...
                    children: [
                              ...
                              Expanded(
                                child: Column(
                                  children: [
                                    ...
                                    Text(
                                      incomeState.cashList.isEmpty
                                          ? "0"
                                          : ref
                                                .watch(cashViewModelProvider)
                                                .cashList
                                                .length
                                                .toString(),
                                      style: AppTextStyle.labelLarge,
                      ...
                      Expanded(
                        child: ListView.builder(
                          itemCount: incomeState.cashList.length,
                          itemBuilder: (context, index) {
                            final item = incomeState.cashList[index];
...
class _SalesPaginationState extends ConsumerState<SalesPagination> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final currentPage = ref.read(cashViewModelProvider).currentPage;
      if (currentPage > 1) {
  ...
  @override
  Widget build(BuildContext context) {
    final purchaseState = ref.watch(cashViewModelProvider);
    final notifier = ref.read(cashViewModelProvider.notifier);
