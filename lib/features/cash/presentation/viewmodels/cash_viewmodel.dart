...
import 'package:pakkahishab/features/cash/data/repositories/cash_repository.dart';
...
final cashViewModelProvider =
    NotifierProvider.autoDispose<CashNotifier, CashState>(
      () => CashNotifier(),
    );
...
class CashNotifier extends Notifier<CashState> {
  late final CashRepository _repo;

  @override
  CashState build() {
    _repo = ref.read(cashRepositoryProvider);
    fetchStock(); // call async stuff manually
  ...
  Future<void> fetchStock({
    ...
    try {
      ...
      final result = await _repo.getCash(
        phone: phone ?? '',
        pin: pin ?? '',
        code: code ?? '',
      ...
      final newItems = items
          .map<CashItem>((e) => CashItem.fromJson(e))
          .toList();
